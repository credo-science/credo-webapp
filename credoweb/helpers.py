...
def get_top_users():
    top = get_redis_connection(write=False).zrevrange(
        cache.make_key("detection_count"), 0, 4, withscores=True
    )
    return [
        {
            "name": u.username,
...
def get_user_list_page(page):
    ...
    if not data:
        ...
        top = [
            (int(t[0]), int(t[1])) for t in top if t[1]
        ]  # Remove users with no detections
...
def get_team_list_page(page):
    ...
    if not data:
        ...
        top = [
            (int(t[0]), int(t[1])) for t in top if t[1]
        ]  # Remove teams with no detections
        teams = {
            team.id: team
            for team in Team.objects.filter(id__in=[t[0] for t in top]).annotate(
...
def get_user_on_time_and_rank(user):
    on_time = get_redis_connection(write=False).zscore(
        cache.make_key("on_time"), user.id
    )
    if not on_time:
        return 0, "no "

    rank = get_redis_connection(write=False).zrevrank(
        cache.make_key("on_time"), user.id
    )
