...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--type", dest="type", help="Type of event to export (detection/ping)"
        )
        ...
        parser.add_argument(
            "--limit", dest="limit", type=int, help="Maximum number of events to export"
        )

        parser.add_argument("--id", dest="id", help="Job ID")
    ...
    def handle(self, *args, **options):
        ...
        data = (
            model.objects.order_by("time_received")
        ...
        with open(settings.EXPORT_TMP_FOLDER + filename, "w") as outfile:
            for chunk in simplejson.JSONEncoder(iterable_as_array=True).iterencode(
                {plural: gen(data.values())}
            ):
                outfile.write(chunk)
